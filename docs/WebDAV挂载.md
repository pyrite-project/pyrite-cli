...
目录缓存只保存目录项和 stat 信息，不缓存完整文件内容。`PUT` 保存文件后会同步更新对应文件 stat 和父目录缓存；`MKCOL`、`DELETE`、`MOVE`、`COPY` 会精确更新或失效相关缓存。若用户通过 REPL、`fs` 命令或其他工具绕过当前 `mount` 会话直接修改设备文件，文件管理器里可能短时间看到旧的目录状态，此时刷新目录或重启 `mount` 会话即可重新同步。
...
缓存是为了改善文件管理器交互体验，不作为设备文件系统的权威状态。需要强一致视图时，优先刷新目录；需要启动时完整目录树更快可见时，使用 `--load-all` 预热。
